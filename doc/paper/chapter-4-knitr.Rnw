...
%Despite that the ranking accuracy shows (table \ref{tab:accuracy}) it is hard to predict complete rankings, the ``top/bottom" analysis suggests that most of the errors in rankings were done at the bottom part.
...
Panel A reports the performance of \market{} (passive strategy). This strategy showed annualized cumulative return of \Sexpr{round(bl.results[8,3,1,1],2)}\% and annualized Sharpe ratio of \Sexpr{round(bl.results[8,3,3,1],3)}. The average number of stocks used per quarter is \Sexpr{round(bl.results[8,3,4,1])} and the turnover ratio of strategy is \Sexpr{round(bl.results[8,3,5,1],3)} which demonstrates the ins/outs of the S\&P 500 constituents list.

Panel B of the table demonstrates the results of trading with rankings based on price target. Observe that consistent with our assumption, the \true{} resulted in the maximum possible annual cumulative return and the Sharpe ratio (\Sexpr{round(bl.results['true',3,1,1],2)}\% and  \Sexpr{round(bl.results['true',3,3,1],3)} respectively). This implies that in the settings where analysts' expected returns and rankings are based on price targets, an investor can gain a maximum results from trading strategy. Given the hypothetical assumption of \true{}, it is not feasible to implement. The next best strategy is
<<pt.max.strat,echo=FALSE,results='asis'>>=
cat(paste0('\\',gsub('[[:punct:]]','',names(which.max(bl.results[2:8,3,1,1]))),'{}'))
@
which is based on our algorithm of predicting the rankings. This strategy resulted in annual cumulative return of \Sexpr{round(max(bl.results[2:8,3,1,1]),2)}\% and the Sharpe ratio of \Sexpr{round(max(bl.results[2:8,3,3,1]),3)}. In addition, the average per quarter turnover ratio of this strategy of \Sexpr{round(max(bl.results[2:8,2,5,1]),3)} implies relative low trading costs.

Panel C of the table demonstrates the results of trading with rankings based on EPS. For this case, the \true{} resulted in the maximum possible annual cumulative return and the Sharpe ratio (\Sexpr{round(bl.results['true',3,1,2],2)}\% and  \Sexpr{round(bl.results['true',3,3,2],3)} respectively). This implies that in the settings where analysts' expected returns and rankings are based on price targets, an investor can gain a maximum results from trading strategy. Given the hypothetical assumption of \true{}, it is not feasible to implement. The next best strategy is
<<eps.max.strat,echo=FALSE,results='asis'>>=
cat(paste0('\\',gsub('[[:punct:]]','',names(which.max(bl.results[2:8,3,1,2]))),'{}'))
@
which is based on our algorithm of predicting the rankings. This strategy resulted in annual cumulative return of \Sexpr{round(max(bl.results[2:8,3,1,2]),2)}\% and the Sharpe ratio of \Sexpr{round(max(bl.results[2:8,3,3,2]),3)}. In addition, the average per quarter turnover ratio of this strategy of \Sexpr{round(max(bl.results[2:8,2,5,2]),3)} implies relative low trading costs.
...
<<brok.ind.var,echo=FALSE,results='asis'>>=
cat('\\midrule','\n')
...
\begin{tabularx}{\linewidth}{r*{5}{Y}}
     ...
     \midrule
<<stock.ind.var,echo=FALSE,results='asis'>>=
...
\begin{tabularx}{\linewidth}{r*{5}{Y}}
     ...
     \midrule
<<glob.ind.var,echo=FALSE,results='asis'>>=
...
results.final <- bl.results[8,3,,1]
...
results.final <- bl.results[1:7,3,,1]
rownames(results.final) <- paste0('\\',gsub('[[:punct:]]|[[:digit:]]','',c(baselines,methods)),'{}')
print(xtable(results.final,display=c('s','f','f','f','d','f'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(bl.results[1:3,1,,])),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
  \end{tabularx}
  \begin{tabularx}{\linewidth}{r*{5}{Y}}
    \midrule
    \multicolumn{5}{l}{\textbf{Panel C: EPS}} \\
    \midrule
<<bl-eps,echo=F,results='asis'>>=
results.final <- bl.results[1:7,3,,2]
rownames(results.final) <- paste0('\\',gsub('[[:punct:]]|[[:digit:]]','',c(baselines,methods)),'{}')
print(xtable(results.final,display=c('s','f','f','f','d','f'),digits=3),only.contents=T,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(bl.results[1:3,1,,])),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
    \bottomrule
