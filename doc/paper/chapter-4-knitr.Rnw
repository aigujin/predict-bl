...
bl.stocks <- intersect(unique(market.set[,Stock]),dimnames(array.all.vvs)[[2]])

ind.v <- setnames(data.table(reshape2::melt(array.all.vvs[,eps.stocks,])),c('Quarter','Stock','Var','value'))
...
<<plot-tr-pmafe,echo=FALSE,include=FALSE,results='hide',fig.width=10,fig.height=8>>=
...
\begin{tabular}{ccccc}
\toprule
<<pt-accuracy,echo=F,results='asis'>>=
...
\begin{table}[ht]
  ...
  \begin{center}
    ...
    \toprule
<<pt-top-table,echo=F,results='asis'>>=
...
<<bl-results-fig,echo=FALSE, include=FALSE,results='hide',fig.width=10,fig.height=8>>=
