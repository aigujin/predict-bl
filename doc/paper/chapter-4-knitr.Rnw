...
bl.results <- acast(unique(melt(data = final.bl,id.vars = c('Method','confAgg'),measure.vars  = c('ann.ret','ann.st','ann.sr','meanViews','Ave.TO')),by=c('Method','variable','confAgg')),Method~confAgg~variable,value.var = 'value')
...
periods <- rbind(final.bl[,data.table(rollapplyr(port.ret,20,roll.ret.f,by=4,partial=F)),by=.(confAgg,Method)][,strat:='yes'],final.bl[,data.table(rollapplyr(ns.port.ret,20,roll.ret.f,by=4,partial=F)),by=.(confAgg,Method)][,strat:='no'],final.bl[,data.table(roll.ret.f(port.ret)),by=.(confAgg,Method)][,strat:='yes'],final.bl[,data.table(roll.ret.f(ns.port.ret)),by=.(confAgg,Method)][,strat:='no'])[,period:=periods.id,by=.(confAgg,Method,strat)][,sr:=ann.ret/ann.sd]


periods.array <- acast(unique(melt(periods,id.vars = c('confAgg','Method','period','strat')),by=c('confAgg','Method','period','strat','variable')),period~confAgg~Method~strat~variable,value.var = 'value')
...
Panel A reports the performance of \market{} (passive strategy). This strategy showed annualized cumulative return of \Sexpr{bl.results[8,3,1]} and annualized Sharpe ratio of \Sexpr{round(bl.results[8,3,3],3)}. The average number of stocks used per quarter is \Sexpr{round(bl.results[8,3,4])} and the turnover ratio of strategy is \Sexpr{round(bl.results[8,3,5],3)} which demonstrates the ins/outs of the S\&P 500 constituents list.

Panel B of the table demonstrates the results of trading with rankings based on price target. Observe that consistent with our assumption, the \true{} resulted in the maximum possible annual cumulative return and the Sharpe ratio ( \Sexpr{bl.results['true',3,1]} and  \Sexpr{bl.results['true',3,3]} respectively). This implies that in the settings where analysts' expected returns and rankings are based on price targets, an investor can gain a maximum results from trading strategy. Given the hypothetical assumption of \true{}, it is not feasible to implement. The next best strategy is
<<max.strat,echo=FALSE,results='asis'>>=
cat(paste0('\\',gsub('[[:punct:]]','',names(which.max(bl.results[2:8,2,1]))),'{}'))
@
which is based on our algorithm of predicting the rankings. This strategy resulted in annual cumulative return of \Sexpr{max(bl.results[2:8,2,1])} and the Sharpe ratio of \Sexpr{max(bl.results[2:8,2,3])}. In addition, the average per quarter turnover ratio of this strategy of \Sexpr{max(bl.results[2:8,2,5])} implies relative low trading costs.
...
results.final <- bl.results[1:7,2,]
